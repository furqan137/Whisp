...
import '../chat/voice_effect_picker.dart';
...
class _GroupChatScreenState extends State<GroupChatScreen> {
  ...
  Future<void> _sendGroupPushNotification(String message) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;

    try {
      await http.post(
        Uri.parse("https://whisp-notify.onrender.com/send-group-notification"),
  ...
  void _openVoiceEffectPicker(File rawAudio) {
    _pendingVoiceFile = rawAudio;
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      // 🔔 ADD THIS LINE
      await _sendGroupPushNotification(text);
  ...
  Future<void> _pickDocumentFiles() async {
    try {
      ...
      if (file != null) {
        ...
        // 🔔 GROUP NOTIFICATION
        await _sendGroupPushNotification("Sent a document 📄");
      }
    } catch (e) {
  ...
  Future<void> _stopRecordingAndSend() async {
    ...
    try {
      ...
      if (path != null) {
        _openVoiceEffectPicker(File(path));
      }
    } catch (e) {
  ...
  Future<void> _sendVoiceMessage(File audioFile) async {
    try {
      ...
      // 🔔 GROUP NOTIFICATION
      await _sendGroupPushNotification("Sent a voice message 🎤");
    } catch (e) {
  ...
  Future<void> _sendSelfDestructMessage(int duration) async {
    ...
    try {
      ...
      if (mediaPath != null) {
        ...
        String notificationText = "Sent a self-destructing message ⏳";

        if (['mp4', 'mov', 'avi', 'mkv'].contains(extension)) {
          notificationText = "Sent a self-destructing video 🎥";
        } else if (['jpg', 'jpeg', 'png', 'webp', 'gif'].contains(extension)) {
          notificationText = "Sent a self-destructing photo 📷";
        } else if (['mp3', 'wav', 'aac', 'm4a'].contains(extension)) {
          notificationText = "Sent a self-destructing voice message 🎤";
        } else {
          notificationText = "Sent a self-destructing file 📄";
        }
        ...
        await _sendGroupPushNotification(notificationText);
      }
      ...
      // =========================
        ...
        // 🔔 GROUP PUSH NOTIFICATION
        await _sendGroupPushNotification("Sent a self-destructing message ⏳");
      }
    } catch (e) {
      setState(() {
        _messageError = e.toString().replaceAll('Exception: ', '');
